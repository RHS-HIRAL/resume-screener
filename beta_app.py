...
with _ref_col:
    if st.button(
        ...
        for _k in [
            ...
            if k in ("sp_resume_groups", "sp_jd_files") or k.startswith("sp_content_")
        ]:
                    ...
                    with st.spinner(f"Downloading `{_chosen_jdf}` from SharePoint…"):
                        try:
                            st.session_state[_jd_cache] = (
                                _make_sp_updater().download_text_content(_jd_item["id"])
                            )
                        except Exception as _dl_err:
                            st.session_state[_jd_cache] = ""
                            st.error(f"❌ Download failed: {_dl_err}")
                ...
                try:
                    ...
                    if _old_score is not None:
                        ...
                        if _status == "OK":
                            ...
                            st.toast(
                                "⚠️ Auto-push failed — use manual button below", icon="⚠️"
                            )
