...
def call_api(
    ...
    try:
        ...
        st.error(
            f"❌ Cannot connect to API at **{endpoint}**. Make sure your FastAPI server is running."
        )
        return None
...
def scan_folder(folder: str, extensions=(".pdf", ".docx", ".txt")) -> list[str]:
    ...
    if not p.exists():
        return []
    return sorted(
        str(f) for f in p.rglob("*") if f.is_file() and f.suffix.lower() in extensions
    )
...
def render_result(result: dict, role: str, resume_filename: str = ""):
    ...
    st.markdown(
        '<div class="rs-section-header">🎯 JD Match Parameters</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.expander("🗂 Raw JSON Response", expanded=False):
        st.markdown(
            f'<div class="json-viewer">{json.dumps(result, indent=2, ensure_ascii=False)}</div>',
            unsafe_allow_html=True,
        )
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown(
        '<span style="font-size:0.75rem;color:#6b7280;text-transform:uppercase;letter-spacing:0.1em;font-weight:600;">📦 Dependencies</span>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown("---")
    st.markdown(
        '<span style="font-size:0.75rem;color:#6b7280;text-transform:uppercase;letter-spacing:0.1em;font-weight:600;">🔗 SharePoint Integration</span>',
        unsafe_allow_html=True,
    )
...
tab_screen, tab_outreach = st.tabs(["🔍  Screen Resumes", "📧  Outreach"])
...
with tab_screen:
    ...
    with _ref_col:
        if st.button(
            "🔄 Refresh", key="sp_refresh", help="Re-fetch file lists from SharePoint"
        ):
            for _k in [
                k
                for k in st.session_state
                if k in ("sp_resume_groups", "sp_jd_files")
                or k.startswith("sp_content_")
            ]:
    ...
    with col_r:
        ...
        st.markdown(
            '<div class="rs-section-header">📄 Resume</div>', unsafe_allow_html=True
        )

        if not sp_enabled:
            st.warning(
                "SharePoint not connected — check .env variables in the sidebar."
            )
        else:
                ...
                if _chosen_sf and _chosen_sf != "— Select job role —":
                    ...
                    if _chosen_rf != "— Select resume —":
                        ...
                        if _r_cache not in st.session_state:
                            with st.spinner(
                                f"Downloading `{_chosen_rf}` from SharePoint…"
                            ):
                                try:
                                    st.session_state[_r_cache] = (
                                        _make_sp_updater().download_text_content(
                                            _r_item["id"]
                                        )
                                    )
                        ...
                        if _r_content:
                            ...
                            st.success(
                                f"✅ {_chosen_rf} — **{len(_r_content.split()):,}** words"
                            )
                            with st.expander("Preview", expanded=False):
                                st.text_area(
                                    "",
                                    value=_r_content[:1500]
                                    + ("…" if len(_r_content) > 1500 else ""),
                                    height=160,
                                    key="preview_resume",
                                    label_visibility="collapsed",
                                )
    ...
    with col_j:
        st.markdown('<div class="rs-card">', unsafe_allow_html=True)
        st.markdown(
            '<div class="rs-section-header">📋 Job Description</div>',
            unsafe_allow_html=True,
        )
            ...
            if not _jd_files:
                ...
                if _chosen_jdf and _chosen_jdf != "— Select JD —":
                    ...
                    if _jd_cache not in st.session_state:
                        with st.spinner(
                            f"Downloading `{_chosen_jdf}` from SharePoint…"
                        ):
                            try:
                    ...
                    if _jd_content:
                        ...
                        st.success(
                            f"✅ {_chosen_jdf} — **{len(_jd_content.split()):,}** words"
                        )
                        with st.expander("Preview", expanded=False):
                            st.text_area(
                                "",
                                value=_jd_content[:1500]
                                + ("…" if len(_jd_content) > 1500 else ""),
                                height=160,
                                key="preview_jd",
                                label_visibility="collapsed",
                            )
    ...
    with st.expander(
        "✏️  Paste Text Directly (override SharePoint selection)", expanded=False
    ):
    ...
    with status_col1:
        ...
        st.markdown(
            f"""<div style="text-align:center;background:#161a24;border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:0.8rem;">
    ...
    if analyse_clicked and ready:
        ...
        if not api_server:
            ...
            with st.spinner(
                "🤖 Sending to AI analysis pipeline… this may take up to 60 seconds"
            ):
                ...
                result = call_api(
                    ...
                    jd_title=st.session_state.get("role_name", "Untitled"),
                )
            ...
            if result:
                ...
                st.session_state["last_resume_filename"] = st.session_state.get(
                    "resume_filename", ""
                )
                ...
                full_name = (
                    result.get("function_2_resume_data_extraction", {})
                ...
                if _sp_enabled and _pdf_fn:
                    _raw_score = result.get("function_1_resume_jd_matching", {}).get(
                    ...
                    try:
                        ...
                        if _old_score is not None:
                            ...
                            if _status == "OK":
                                ...
                                st.toast(
                                    "⚠️ Auto-push failed — use manual button below",
                                    icon="⚠️",
                                )
                    except Exception as _push_err:
                        st.session_state["auto_push_status"] = ("error", str(_push_err))
                        st.toast(
                            "⚠️ Auto-push error — use manual button below", icon="⚠️"
                        )
                elif _sp_enabled and not _pdf_fn:
                    st.session_state["auto_push_status"] = (
                        "skipped",
                        "No filename detected — use manual push below.",
                    )
        ...
        if _sc:
            ...
            def _do_score_push(chosen: int) -> None:
                ...
                try:
                    ...
                    if _s == "OK":
                        ...
                        st.toast(
                            f"📤 MatchScore = {chosen} saved to SharePoint", icon="✅"
                        )
                    else:
            ...
            with _btn_col4:
                if st.button(
                    ...
                    st.session_state["auto_push_status"] = (
                        "skipped",
                        "Score comparison dismissed — use manual push button below.",
                    )
                    st.rerun()
    ...
    st.markdown("")
    with st.expander(
        "📁  Browse Saved Results (extracted_json_output/)", expanded=False
    ):
        ...
        if not out_base.exists():
            ...
            saved_files = sorted(
                out_base.rglob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True
            )
            if not saved_files:
                st.info("No saved results yet.")
            else:
                ...
                for sf in saved_files[:30]:  # Show latest 30
                    ...
                    with cols[0]:
                        st.markdown(f"📄 `{rel}`")
                    with cols[1]:
                        mtime = datetime.fromtimestamp(sf.stat().st_mtime).strftime(
                            "%m/%d %H:%M"
                        )
                        st.caption(mtime)
                    with cols[2]:
...
with tab_outreach:
    render_outreach_tab(st.session_state.get("api_url", "http://localhost:8000"))
