...
def main():
    ...
    if st.button("🚀 Start Accurate Processing", type="primary"):
        ...
        with st.status("🚀 Orchestrating AI Extraction...", expanded=True) as status:
            progress_bar = st.progress(0)
            current_file_display = st.empty()

            for idx, resume_file in enumerate(resume_files, 1):
                # Update display with current file info in a clean markdown format
            ...
            status.update(
                label="✅ AI Analysis Complete!", state="complete", expanded=False
            )
